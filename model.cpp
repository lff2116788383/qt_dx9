    ...
    for(unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
       ...
       {   // 如果材质还没有加载，加载它
           ...
           if(!data.isNull()){
               ...
               //qDebug() << "成功加载纹理：" << directory.filePath(str.C_Str());
           }
           else{
...
void Model::setTextures(QVector<QString>& TexturesPath)
{
    textures_loaded.clear();
    Texture* texture = new Texture;
    for (int i = 0; i < TexturesPath.size(); i++)
    {
        QImage data(TexturesPath[i]);
        if (!data.isNull()) {
            texture->texture.setData(data);
            texture->type = QString("default");;
            texture->path = TexturesPath[i];
...
//};
//
//void processBone(const aiMesh* pMesh, vector<BoneVertexData>& vertices, bool bAdd)
//{

...
bool isWireFrame=false; //是否线框渲染模式
...
void D3d11RenderWidget::CreateBox(GeometryGenerator::MeshData& meshData)
{
	// 顶点、偏移信息
	mBoxVertexOffset = 0;
	mBoxIndexCount = meshData.Indices.size();
	mBoxIndexOffset = 0;

	totalVertexCount = meshData.Vertices.size(); //sizeof(v) / sizeof(Vertex);
	totalIndexCount =
		mBoxIndexCount;
	...
	for (size_t i = 0; i < meshData.Vertices.size(); i++)
	{
		...
		vertices[i].color.x= float(rand()%2);
		vertices[i].color.y = float(rand() % 2);
		vertices[i].color.z = float(rand() % 2);
	...
	qDebug() << "mBoxIndexCount:" << mBoxIndexCount << "totalVertexCount:" << totalVertexCount;
	...
	indexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
	indexBufferDesc.ByteWidth = sizeof(DWORD) * totalIndexCount;//sizeof(DWORD) * 12 * 3;//12个索引
	indexBufferDesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
	indexBufferDesc.CPUAccessFlags = 0;
	indexBufferDesc.MiscFlags = 0;
	...
	vertexBufferDesc.Usage = D3D11_USAGE_DEFAULT;
	vertexBufferDesc.ByteWidth = sizeof(Vertex) * totalVertexCount;//八个顶点
	vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vertexBufferDesc.CPUAccessFlags = 0;
	vertexBufferDesc.MiscFlags = 0;
...
void D3d11RenderWidget::RenderScene()
{
	...
	//Draw the first cube
	m_d3dDevContext->DrawIndexed(mBoxIndexCount, mBoxIndexOffset, mBoxVertexOffset);// (36, 0, 0);
